...
const (
	WINPTY_FLAG_PLAIN_OUTPUT        = uint64(2)
...
func New(winPTYDLLDir string) (*WinPTY, error) {
	...
	agentCfg, _, _ := winpty_config_new.Call(uintptr(uint64(WINPTY_FLAG_PLAIN_OUTPUT)), uintptr(unsafe.Pointer(&errorPtr)))
	if agentCfg == 0 {
...
func (w *WinPTY) Run(cmd *exec.Cmd) error {
	escaped := makeCmdLine(append([]string{}, cmd.Args...))
